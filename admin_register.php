...
if (isset($_POST['submit'])) {
    ...
    $user_type = "admin"; // Set the default value of user_type as "user"
    $age = mysqli_real_escape_string($conn, $_POST['age']); // Add this line to get age value

    if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM admin WHERE email='{$adminemail}'")) > 0) {
        $msg = "<div class='alert alert-danger'>{$adminemail} - This email address has already been used.</div>";
    } else {
        if ($password === $cpassword) {
            $sql = "INSERT INTO admin (username, email, password, code, user_type, age) VALUES ('{$name}', '{$adminemail}', '{$password}', '{$code}', '{$user_type}', '{$age}')";
					...
					<div class="register-page">
						...
						<div class="login-box">
							<div class="block-title">
								<h2 class="title" style="color:#78b144;"><span>Create Admin Account</span></h2>
							</div>
							...
							<?php echo $msg; ?>
								...
								<div class="form-group">
									<label>Email</label>
									<input type="email" value="<?php if (isset($_POST['submit'])) { echo $adminemail; }?>" name="email" required>
								</div>
								<div class="form-group">
									<label>Age</label>
								...
								<div class="form-group text-center">
									...
									<p>Already have an account? <a href="adminlogin.php" style="color: #78b144;">LOGIN NOW</a></p>
